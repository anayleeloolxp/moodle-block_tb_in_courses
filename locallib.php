...
/**
 * Helper functions for tb_in_courses block
 *
...
define('BLOCKS_TB_IN_COURSES_SHOWCATEGORIES_NONE', '0');
define('BLOCKS_TB_IN_COURSES_SHOWCATEGORIES_ONLY_PARENT_NAME', '1');
define('BLOCKS_TB_IN_COURSES_SHOWCATEGORIES_FULL_PATH', '2');
define('BLOCKS_TB_IN_COURSES_IMAGEASBACKGROUND_FALSE', '0');
define('BLOCKS_TB_IN_COURSES_SHOWGRADES_NO', '0');
define('BLOCKS_TB_IN_COURSES_SHOWGRADES_YES', '1');
define('BLOCKS_TB_IN_COURSES_STARTGRID_NO', '0');
define('BLOCKS_TB_IN_COURSES_STARTGRID_YES', '1');
define('BLOCKS_TB_IN_COURSES_DEFAULT_COURSES_ROW', '4');
define('BLOCKS_TB_IN_COURSES_DEFAULT_COL_SIZE', '3');
define('BLOCKS_TB_IN_COURSES_SHOWTEACHERS_NO', '0');
define('BLOCKS_TB_IN_COURSES_SHOWTEACHERS_YES', '1');
 ...
 */
function block_tb_in_courses_get_overviews($courses) {
    $htmlarray = array();
...
/**
 * Sets user preference for maximum courses to be displayed in tb_in_courses block
 *
 * @param int $number maximum courses which should be visible
 */
function block_tb_in_courses_update_mynumber($number) {
    set_user_preference('tb_in_courses_number_of_courses', $number);
}

/**
 * Sets user course sorting preference in tb_in_courses block
 *
 * @param array $sortorder list of course ids
 */
function block_tb_in_courses_update_myorder($sortorder) {
    ...
    set_user_preference('tb_in_courses_course_sortorder', $value);
}

/**
 * Gets user course sorting preference in tb_in_courses block
 *
 * @return array list of course ids
 */
function block_tb_in_courses_get_myorder() {
    if ($value = get_user_preferences('tb_in_courses_course_sortorder')) {
        return explode(',', $value);
    }
    // If preference was not found, look in the old location and convert if found.
    $order = array();
    if ($value = get_user_preferences('tb_in_courses_course_order')) {
        $order = unserialize_array($value);
        block_tb_in_courses_update_myorder($order);
        unset_user_preference('tb_in_courses_course_order');
    }
    return $order;
}
 ...
 */
function block_tb_in_courses_get_child_shortnames($courseid) {
    global $DB;
    ...
    if ($results = $DB->get_records_sql($sql, $params)) {
        ...
        if ($total > 10) {
            ...
            if ($diff > 1) {
                $suffix = get_string('shortnamesufixprural', 'block_tb_in_courses', $diff);
            } else {
                $suffix = get_string('shortnamesufixsingular', 'block_tb_in_courses', $diff);
            }
        }
        $shortnames = get_string('shortnameprefix', 'block_tb_in_courses', implode('; ', $shortnames));
        $shortnames .= $suffix;
    }
...
/**
 * Returns maximum number of courses which will be displayed in tb_in_courses block
 *
 * @param bool $showallcourses if set true all courses will be visible.
 * @return int maximum number of courses
 */
function block_tb_in_courses_get_max_user_courses($showallcourses = false) {
    // Get block configuration.
    $leeloolxplicense = get_config('block_tb_in_courses')->license;
    ...
    if (!$output = $curl->post($url, $postdata, $options)) {
        $limit = get_user_preferences('tb_in_courses_number_of_courses', $limit);
    }
    ...
    $settingleeloolxp = $resposedata->data->inprogress_courses;

    $limit = @$settingleeloolxp->inprogress_defaultmaxcourses;

    // If max course is not set then try get user preference.
    if (empty($settingleeloolxp->inprogress_forcedefaultmaxcourses)) {
        if ($showallcourses) {
            $limit = 0;
        } else {
            $limit = get_user_preferences('tb_in_courses_number_of_courses', $limit);
 ...
 */
function block_tb_in_courses_get_sorted_courses($showallcourses = false, $categoryid = 1) {
    global $USER;
    global $DB;

    $categorythispath = '/' . $categoryid . '/';

    $limit = block_tb_in_courses_get_max_user_courses($showallcourses);
    ...
    $order = block_tb_in_courses_get_myorder();
 ...
 * @return string
 */
function block_tb_in_courses_build_progress($course, $config) {
    global $CFG;
    ...
    if (@$config->inprogress_progressenabled == BLOCKS_TB_IN_COURSES_SHOWGRADES_NO) {
        return '';
    }
 ...
 * @return string
 */
function block_tb_in_courses_progress_percent($course) {
    global $CFG;
 ...
 */
function updateconfin_courses() {
    if (isset(get_config('block_tb_in_courses')->license)) {
        $leeloolxplicense = get_config('block_tb_in_courses')->license;
    } else {
        return;
    }
