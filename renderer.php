...
/**
 * tb_in_courses block rendrer
 *
 * @package   block_tb_in_courses
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_tb_in_courses_renderer extends plugin_renderer_base {

    /**
     * Construct contents of tb_in_courses block
     *
     * @param array $courses list of courses in sorted order
     * @param stdClass|stdObject $config settings from Leeloo
     * @return string html to be displayed in tb_in_courses block
     */
    public function tb_in_courses($courses, $config) {
        global $CFG, $DB;
        $html = '';
        // LearningWorks.
        ...
        if($config->inprogress_showasslider == 1){
            ...
            $this->page->requires->js(new moodle_url($CFG->wwwroot . '/blocks/tb_in_courses/js/owlslider.js'));

            $this->page->requires->css(new moodle_url($CFG->wwwroot . '/blocks/tb_in_courses/css/owl.carousel.min.css'));
            $this->page->requires->css(new moodle_url($CFG->wwwroot . '/blocks/tb_in_courses/css/owl.theme.default.min.css'));

        }

        $this->page->requires->js(new moodle_url($CFG->wwwroot . '/blocks/tb_in_courses/js/custom.js'));
        ...
        if ($this->page->user_is_editing() && (count($courses) > 1)) {
            $userediting = true;
            $this->page->requires->js_init_call('M.block_tb_in_courses.add_handles');
        ...
        if ($ismovingcourse) {
            ...
            $html .= get_string('movingcourse', 'block_tb_in_courses', $a);
            $html .= $this->output->box_end();

            $moveurl = new moodle_url('/blocks/tb_in_courses/move.php',
            ...
            $movetofirsticon = html_writer::empty_tag('img',
                array('src' => $moveicon,
                    'alt' => get_string('movetofirst', 'block_tb_in_courses', $courses[$movingcourseid]->fullname),
        ...
        $fields = 'u.id, u.confirmed, u.username, ' . $allnames . ', ' .
            ...
            if($config->inprogress_showasslider == 1){    
                $html .= html_writer::start_div('tb_in_courses_list owl-carousel owl-theme');
            }else{
                $html .= html_writer::start_div('tb_in_courses_list');
            }
        foreach ($courses as $key => $course) {
            ...
            $teachernames = '';
            if ($course->id > 0 && !empty($role) && $config->inprogress_showteachers != BLOCKS_TB_IN_COURSES_SHOWTEACHERS_NO) {
            ...
            if ($userediting && !$ismovingcourse) {
                $moveicon = html_writer::empty_tag('img',
                    array('src' => $moveicon->out(false),
                        'alt' => get_string('movecourse', 'block_tb_in_courses', $course->fullname),
            ...
            if (!empty($config->inprogress_showchildren) && ($course->id > 0)) {
                // List children here.
                if ($children = block_tb_in_courses_get_child_shortnames($course->id)) {
            ...
            if ($course->id > 0) {
                ...
                $html .= block_tb_in_courses_build_progress($course, $config);

                $html .= html_writer::div($teachernames, 'teacher_names');
            }

            if ($config->inprogress_showcategories != BLOCKS_TB_IN_COURSES_SHOWCATEGORIES_NONE) {
                ...
                if ($currentcategory !== null) {
                    ...
                    if ($config->inprogress_showcategories == BLOCKS_TB_IN_COURSES_SHOWCATEGORIES_FULL_PATH) {
            ...
            if ($ismovingcourse) {
                ...
                $movehereicon = html_writer::empty_tag('img',
                    array('src' => $movehere,
                        'alt' => get_string('moveafterhere', 'block_tb_in_courses', $a),
                        'title' => get_string('movehere')));
    ...
    protected function activity_display($cid, $overview) {
        ...
        foreach (array_keys($overview) as $module) {
            ...
            if (get_string_manager()->string_exists("activityoverview", $module)) {
                ...
                $icontext .= get_string("activityoverview", 'block_tb_in_courses', $modulename);
            }
    ...
    public function editing_bar_head($max = 0) {
        ...
        $options = array('0' => get_string('alwaysshowall', 'block_tb_in_courses'));
        for ($i = 1; $i <= $max; $i++) {
            $options[$i] = $i;
        }
        $url = new moodle_url('/my/index.php');
        $select = new single_select($url, 'mynumber', $options, block_tb_in_courses_get_max_user_courses(), array());
        $select->set_label(get_string('numtodisplay', 'block_tb_in_courses'));
    ...
    protected function collapsible_region_start($classes, $id, $caption, $userpref = '', $default = false) {
        ...
        $this->page->requires->js_init_call('M.block_tb_in_courses.collapsible', array($id, $userpref, get_string('clicktohideshow')));
    ...
    public function welcome_area($msgcount) {
        ...
        $output .= $this->output->heading(get_string('welcome', 'block_tb_in_courses', $USER->firstname));

        if (!empty($CFG->messaging)) {
            $plural = 's';
            if ($msgcount > 0) {
                $output .= get_string('youhavemessages', 'block_tb_in_courses', $msgcount);
                if ($msgcount == 1) {
                    $plural = '';
                }
            } else {
                $output .= get_string('youhavenomessages', 'block_tb_in_courses');
            }
            $output .= html_writer::link(new moodle_url('/message/index.php'),
                get_string('message' . $plural, 'block_tb_in_courses'));
        }
        $output .= $this->output->box_end();
    ...
    public function course_image($course, $config) {
        ...
        if ($course->id > 0 && $course->get_course_overviewfiles()) {
            foreach ($course->get_course_overviewfiles() as $file) {
                ...
                if ($isimage) {
                    if (is_null($config->inprogress_tb_a_courses_bgimage) ||
                        $config->inprogress_tb_a_courses_bgimage == BLOCKS_TB_IN_COURSES_IMAGEASBACKGROUND_FALSE) {
    ...
    public function course_image_defaults($config) {
        ...
        if (method_exists($this->output, 'image_url')) {
            // Use the new method.
            $default = $this->output->image_url('default', 'block_tb_in_courses');
        } else {
            // Still a pre Moodle 3.3 release. Use pix_url because image_url doesn't exist yet.
            $default = $this->output->pix_url('default', 'block_tb_in_courses');
        }
        if ($courseimagedefault = $config->inprogress_courseimagedefault) {
            // Return an img element with the image in the block settings to use for the course.
            $imageurl = $courseimagedefault;
        } else {
            // We check for a default image in the tb_in_courses pix folder named default aka our final hope.
            $imageurl = $default;
        }

        // Do we need a CSS soloution or is a img good enough?.
        if (is_null($config->inprogress_tb_a_courses_bgimage) || $config->inprogress_tb_a_courses_bgimage == BLOCKS_TB_IN_COURSES_IMAGEASBACKGROUND_FALSE) {
            // Embed the image url as a img tag sweet...
    ...
    public function course_description($course, $config) {
        ...
        $limit = $config->inprogress_summary_limit;
        if ($limit == '') {
            $limit = 200;
        }
